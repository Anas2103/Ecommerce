  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-page)' }}>
      ...
      <aside className="w-56 flex flex-col flex-shrink-0" style={{ background: 'var(--bg-card)', borderRight: '1.5px solid var(--border-light)' }}>
        ...
        {/* Brand */}
        <div style={{ padding: '16px', borderBottom: '1.5px solid var(--border-light)', background: '#0066CC' }}>
        ...
        <div style={{ padding: '12px', borderTop: '1.5px solid var(--border-light)' }}>
          <div className="flex items-center gap-2.5 mb-2">
            <img
              src={user?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'S')}&background=0066CC&color=fff&size=64`}
              alt=""
              style={{ width: 30, height: 30, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }}
            />
            <div className="flex-1 min-w-0">
              <p style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-1)' }} className="truncate">{user?.name}</p>
              <p style={{ fontSize: '0.65rem', color: '#0066CC' }}>Seller</p>
            </div>
          </div>
          <button
