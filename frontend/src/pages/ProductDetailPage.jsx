  ...
  return (
    <div style={{ background: 'var(--bg-page)', minHeight: '100vh' }}>
      <div style={{ padding: '32px 24px' }}>
        ...
        {/* Breadcrumb */}
        <nav style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.8125rem', marginBottom: 24, color: 'var(--text-3)' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
          ...
          <div className="space-y-3">
            <div style={{ position: 'relative', aspectRatio: '1', overflow: 'hidden', borderRadius: 16, background: 'var(--bg-card)', border: '1.5px solid var(--border)' }}>
              <img src={currentImage} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} onError={(e) => e.target.src = '/placeholder.jpg'} />
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    style={{
                      ...
                      border: i === activeImage ? '2px solid #0066CC' : '1.5px solid var(--border)',
                      boxShadow: i === activeImage ? '0 0 0 3px rgba(0,102,204,0.14)' : 'none',
          ...
          <div className="space-y-4">
            {product.category && (
              <Link
                ...
                style={{ display: 'inline-flex', fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#0066CC', textDecoration: 'none' }}
              >
                {product.category.name}
              </Link>
            )}
            <h1 style={{ fontSize: '1.875rem', fontWeight: 800, color: 'var(--text-1)', margin: 0 }}>{product.name}</h1>
            ...
            {product.stock > 0 && (
              <div className="space-y-3">
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  <button onClick={handleAddToCart} disabled={addingToCart} className="btn-primary" style={{ flex: 1, padding: '12px 20px', fontSize: '1rem', justifyContent: 'center', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: inWishlist ? '#0066CC' : 'var(--text-3)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      transition: 'all 0.15s',
                    }}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--border-accent)'; e.currentTarget.style.color = '#0066CC' }}
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = inWishlist ? 'var(--border-accent)' : 'var(--border)'; e.currentTarget.style.color = inWishlist ? '#0066CC' : 'var(--text-3)' }}
                  >
            ...
            {/* Shipping info */}
            <div style={{ paddingTop: 16, borderTop: '1.5px solid var(--border-light)', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.875rem', color: 'var(--text-2)' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', gap: 24, marginBottom: 24, borderBottom: '1.5px solid var(--border-light)' }}>
            {['description', 'reviews', 'specifications'].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                style={{
                  paddingBottom: 12, fontSize: '0.875rem', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer',
                  borderBottom: activeTab === tab ? '2px solid #0066CC' : '2px solid transparent',
                  color: activeTab === tab ? '#0066CC' : 'var(--text-3)',
          ...
          {activeTab === 'reviews' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {product.average_rating > 0 && (
                <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-light)', borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column', gap: 24, boxShadow: 'var(--shadow-card)' }} className="sm:flex-row">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {product.reviews?.length === 0 ? (
                  ...
                  <div key={review.id} style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-light)', borderRadius: 12, padding: 16, boxShadow: 'var(--shadow-card)' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                      <img
                        src={review.user?.avatar ? `${import.meta.env.VITE_STORAGE_URL}/${review.user.avatar}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(review.user?.name || 'U')}&background=0066CC&color=fff`}
                        alt=""
        ...
        {related.length > 0 && (
          <section style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 800, marginBottom: 24, color: 'var(--text-1)' }}>{t('products.related')}</h2>
